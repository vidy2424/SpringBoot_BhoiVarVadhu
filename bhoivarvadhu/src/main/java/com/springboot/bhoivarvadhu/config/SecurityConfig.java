...
import com.springboot.bhoivarvadhu.security.JWTAuthenticationFilter;
import com.springboot.bhoivarvadhu.security.JWTAuthorizationFilter;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	// login form instead auto generated login form by spring security
	// The updated configuration specifies the location of the log in page.
	// We must grant all users (i.e. unauthenticated users) access to our log in
	// page
	/*
	...
	// Any URL that starts with "/admin/" will be restricted to users who have the
	// role "ROLE_ADMIN".
	// Any URL that starts with "/db/" requires the user to have both "ROLE_ADMIN"
	// and "ROLE_DBA".
...
// 	
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.cors().and().csrf().disable().authorizeRequests().antMatchers("/**").permitAll().antMatchers("/")
				.hasRole("anonymousUser").anyRequest().authenticated().and().sessionManagement()
	 ...
	 * cookie = WebUtils.getCookie(request, "XSRF-TOKEN"); String token =
	 * csrf.getToken(); if (cookie == null || token != null &&
	 * !token.equals(cookie.getValue())) { cookie = new Cookie("XSRF-TOKEN", token);
...
//				Arrays.asList("Authorization", "Cache-Control", "Content-Type", "xsrfheadername", "xsrfcookiename",
...
//	}
	 ...
	 * throws Exception { auth .inMemoryAuthentication() //Adds a
	 * UserDetailsManagerConfigurer //login, password and supported role
